...
class NotEnabled(RuntimeError):
    ...
    def __init__(self, server: str):
        ...
        super().__init__(
            ...
            f"Tell the user to run `/mcp login {server}` (or `/login` → Services) "
            f"in Devo to connect it. Do not ask them to set environment variables."
        )
...
def _agent_dir() -> Path:
    """Resolve the Devo/Prime Agent config dir the same way the host does."""
    raw = (
...
def _resolve_streamable_http():
    ...
    try:
        ...
        raise RuntimeError(
            "The MCP Python SDK (`mcp`) is not installed in this kernel's "
            "Python environment, so Linear/Notion-style integrations cannot "
            "connect. Install it with: python -m pip install 'mcp>=2,<3'"
        ) from error
