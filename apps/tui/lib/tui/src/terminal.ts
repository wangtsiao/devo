...
export class ProcessTerminal implements Terminal {
	...
	/** True while we own terminal modes that must be restored on process death. */
	private processExitRestoreInstalled = false;
	...
	private readonly onProcessExit = (): void => {
		this.restoreTerminalForProcessExit();
	};
	...
	start(onInput: (data: string) => void, onResize: () => void): void {
		...
		// Sync restore if the process dies without stop() (signals, crash exit,
		// process.exit). SIGKILL cannot be handled; everything else should recover.
		this.installProcessExitRestore();
	}

	private installProcessExitRestore(): void {
		if (this.processExitRestoreInstalled) return;
		this.processExitRestoreInstalled = true;
		process.on("exit", this.onProcessExit);
	}

	private uninstallProcessExitRestore(): void {
		if (!this.processExitRestoreInstalled) return;
		this.processExitRestoreInstalled = false;
		process.off("exit", this.onProcessExit);
	}

	/**
	 * Best-effort synchronous tty restore for abnormal process death.
	 * Must not throw: Node exit handlers that throw are ignored and leave
	 * the parent shell in raw mode with no echo.
	 */
	private restoreTerminalForProcessExit(): void {
		try {
			...
			this.uninstallProcessExitRestore();
			this.started = false;
		}
	}
	...
	stop(options: TerminalStopOptions = {}): void {
		...
		if (options.preserveAltScreen && wasStarted) {
			...
			// Keep exit restore installed: if the process dies mid-handoff the
			// parent shell would otherwise be left in raw mode.
		} else {
			...
			// Restore raw mode state before dropping the exit safety net.
			if (process.stdin.setRawMode) {
				process.stdin.setRawMode(this.wasRaw);
			}
			this.uninstallProcessExitRestore();
