...
function extractWebsearchQuery(code: string): string | undefined {
	// Single capturing group for the opening quote so `\1` closes the same style
	// and the query stays in match[2] (avoid nested `(("""|…))` which stole group 2).
	const match = code.match(
		/websearch(?:\.run)?\s*\(\s*(?:query\s*=\s*)?[rRfFbBuU]*("""|'''|"|')([\s\S]*?)\1/,
	);
	const query = match?.[2]?.replace(/\s+/g, " ").trim();
	if (!query) return undefined;
	const short = query.length > 56 ? `${query.slice(0, 55).trimEnd()}…` : query;
	return `websearch '${short}'`;
}

function simplifyPythonPreviewLine(line: string, paths: ReadonlyMap<string, string>): string {
	return (
		extractWebsearchQuery(line) ??
		pythonFileOperation(line, paths) ??
		pythonSubprocessCommand(line) ??
		pythonPrintInnerCall(line) ??
		line.trim()
	);
}
...
export function previewPythonCode(code: string): CodePreview {
	...
	if (bestIndex !== undefined && bestScore >= 0) {
		...
		const websearch = extractWebsearchQuery(
			[lines[previewIndex], ...rawLines.slice(previewIndex + 1)].join("\n"),
		);
		if (websearch) {
			return { language: "python", text: websearch };
		}
		return {
			language: "python",
