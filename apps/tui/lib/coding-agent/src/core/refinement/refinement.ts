...
const REFINEMENT_SYSTEM_PROMPT = `You are Devo's /refine continual harness subsystem.
...
skills, and subagent specs that lets Devo improve reusable behavior
...
- The default editable continual harness store is local to the current Devo session. Use it for session-specific progress, active task state, current-run coordination notes, temporary blockers, and project facts that should not affect other sessions.
...
const AUTO_REFINE_REVIEW_SYSTEM_PROMPT = `You are Devo's automatic /refine review gate.
...
export function formatHarnessStateForPrompt(
	...
	const lines = [
		...
		"",
		"Local continual harness entries belong to this Devo session. Global continual harness entries persist across Devo sessions.",
