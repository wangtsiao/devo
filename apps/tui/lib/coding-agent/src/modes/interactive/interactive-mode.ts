...
import { type FileChangeSummary, formatTotalChangeSummary } from "./components/edit-summary.js";
...
import { SystemPromptComponent } from "./components/system-prompt.js";
import {
...
import { ExpandableText, isExpandable, hasEditDiffsExpansion } from "./expandable-text.js";
export { ExpandableText } from "./expandable-text.js";
export { formatQueuedMessagePreview, styleQueuedMessagePreview } from "./queued-preview.js";
import { formatQueuedMessagePreview, styleQueuedMessagePreview } from "./queued-preview.js";
...
export {
	BrandSplashHeader,
	type BrandSplashHeaderOptions,
	type BrandSplashMetadataLine,
	formatSplashCwd,
	truncatePathMiddle,
} from "./brand-splash-header.js";
import {
	BrandSplashHeader,
	formatSplashCwd,
	truncatePathMiddle,
} from "./brand-splash-header.js";
import { mergeSubagentSnapshot } from "./brand-splash-header.js";
...
export interface InteractiveModeOptions {
	...
	/**
	 * When handing off to the agents view, leave `agentConnection` open so a
	 * Native (non-daemon) host can switch sessions without reconnecting.
	 */
	preserveAgentConnectionOnHandoff?: boolean;
	...
	/** Override splash logo/title metadata (e.g. Devo product branding). */
	brandSplash?: Pick<BrandSplashHeaderOptions, "logo" | "title">;
	/** Display / update-check version label. Defaults to package VERSION. */
	version?: string;
	/** Terminal window title prefix. Defaults to APP_TITLE. */
	appTitle?: string;
	/**
	 * Provider ids omitted from /login and the providers configuration tab.
	 * Devo sets this to exclude Prime Inference branding from the login surface.
	 */
	excludedLoginProviderIds?: ReadonlyArray<string>;
	/**
	 * Builtin slash command names the host serves itself. Excluded builtins are
	 * hidden from autocomplete and skipped by the local command dispatch, so they
	 * fall through to `agentConnection.prompt()` (e.g. Devo's `/traces`).
	 */
...
export class InteractiveMode {
	...
	/** Lazily built lookup for `excludedBuiltinCommands` (host-owned builtins). */
	private excludedBuiltinCommandNames?: Set<string>;
	...
	constructor(private options: InteractiveModeOptions) {
		...
		this.appTitle = options.appTitle ?? APP_TITLE;
	...
	private getBuiltInCommandConflictDiagnostics(
		...
		return commands
			...
			.filter((command) => this.isEffectiveBuiltinSlashCommandName(command.registeredName ?? command.name))
			.map((command) => ({
	...
	/**
	 * Whether `name` resolves to a builtin this host still serves locally.
	 * Excluded builtins stay recognized command names but are not executed or
	 * listed by InteractiveMode, so the host transport can serve them instead.
	 */
	private isEffectiveBuiltinSlashCommandName(name: string): boolean {
		return isBuiltinSlashCommandName(name) && !this.isExcludedBuiltinCommand(name);
	}

	private isExcludedBuiltinCommand(name: string): boolean {
		this.excludedBuiltinCommandNames ??= new Set(
			(this.options.excludedBuiltinCommands ?? []).map((entry) =>
				resolveBuiltinSlashCommandName(entry.startsWith("/") ? entry.slice(1) : entry),
			),
		);
		const canonical = resolveBuiltinSlashCommandName(name.startsWith("/") ? name.slice(1) : name);
		return this.excludedBuiltinCommandNames.has(canonical);
	}
	...
	private createBaseAutocompleteProvider(): AutocompleteProvider {
		const slashCommands: SlashCommand[] = BUILTIN_SLASH_COMMANDS.filter(
			...
			.filter((command) => !this.isExcludedBuiltinCommand(command.name))
			.map((command) => ({
				name: command.name,
		...
		if (themeCommand) {
			themeCommand.getArgumentCompletions = (prefix: string): AutocompleteItem[] | null =>
				this.getThemeCompletions(prefix);
		}
		...
		const extensionCommands: SlashCommand[] = connectionCommands
			...
			.filter((cmd) => !this.isEffectiveBuiltinSlashCommandName(cmd.name))
			.map((cmd) => ({
				name: cmd.name,
				description: cmd.description,
				sourceTag: this.getAutocompleteSourceLabel(cmd.sourceInfo),
	...
	private renderRecap(): void {
		...
		// Show as soon as tools report file changes (not only after agent_end).
		const showChanges = this.agentRunFileChanges.size > 0;
		if (showChanges) {
	...
	/**
	 * Thin path extraction only: expand `~` and collect absolute image paths for
	 * Native `LocalImage` inputs. File read/encode belongs on the server.
	 * Supports quoted paths and unquoted absolute paths that contain spaces
	 * (e.g. `C:\Users\…\Screenshot 2026-09-16 192230.png`).
	 */
	private extractLocalImagePathsFromText(text: string): {
		...
		// Prefer quoted paths; otherwise allow spaces until an image extension.
		...
		for (const match of matches.reverse()) {
			...
			localImages.unshift({ path: expanded });
			const full = match[0]!;
			const pathOffset = full.indexOf(rawPath);
			let start = match.index! + pathOffset;
			let end = start + rawPath.length;
			// Drop surrounding quotes when the match used a quoted form.
			if (
				start > 0 &&
				(out[start - 1] === '"' || out[start - 1] === "'") &&
				out[end] === out[start - 1]
			) {
				start -= 1;
				end += 1;
		...
		return { text: out.trim(), localImages };
	}
	...
	private setupEditorSubmitHandler(): void {
		this.defaultEditor.onSubmit = async (text: string) => {
			...
			try {
				...
				// Host-owned builtins (e.g. Devo /traces) are not executed locally;
				// they fall through to agentConnection.prompt() as plain text.
				const commandName =
					slashCommand && !this.isExcludedBuiltinCommand(slashCommand.name)
						? resolveBuiltinSlashCommandName(slashCommand.name)
						: undefined;
				...
				if (commandName === "theme") {
					this.editor.setText("");
					this.handleThemeCommand(commandArgs);
					return;
				}
				...
				this.flushPendingBashComponents();
				const model = this.getCurrentModel();
				const supportsImages = !model || model.input.includes("image");
				const { text: textWithoutPaths, localImages } = this.extractLocalImagePathsFromText(text);
				if (localImages.length > 0 && !supportsImages) {
					this.showStatus("Current model does not support images; the attachment will be omitted.");
				}
				const pastedImages = this.collectImagesFor(textWithoutPaths);
				const images =
					supportsImages && localImages.length > 0
						? [...(pastedImages ?? []), ...localImages]
						: pastedImages;
				const promptText = textWithoutPaths.length > 0 ? textWithoutPaths : text;
				this.editor.addToHistory?.(text);
				this.editor.setText("");
				...
				try {
					await this.agentConnection.prompt(promptText, {
						streamingBehavior,
						queueIfBusy: true,
						images,
					});
	...
	private async handleEvent(event: AgentConnectionSessionEvent): Promise<void> {
		...
		switch (event.type) {
			...
			case "bash_start": {
				if (this.activeBashComponent) {
					// Optimistic local start plus command/exec must share one chrome.
					break;
				}
			...
			case "tool_pending": {
				// Args may still be streaming — queued / "waiting for code", not running.
				await this.getOrCreatePendingToolComponent({
					id: event.toolCallId,
					name: event.toolName,
			...
			case "tool_args_update": {
				...
				if (component) {
					...
					// Delta arrived before pending — create with current args, still not running.
					await this.getOrCreatePendingToolComponent({
						id: event.toolCallId,
			...
			case "tool_args_complete": {
				if (event.toolCallId) {
					this.pendingTools.get(event.toolCallId)?.setArgsComplete();
				} else {
					for (const [, component] of this.pendingTools.entries()) {
						component.setArgsComplete();
					}
			...
			case "tool_execution_start": {
				...
				} else if (event.args && typeof event.args === "object" && Object.keys(event.args).length > 0) {
					component.updateArgs(event.args);
				}
				component?.setArgsComplete();
				component?.markExecutionStarted();
			...
			case "tool_execution_end": {
				const component = this.pendingTools.get(event.toolCallId);
				if (component) {
					// Per-tool file diffs live on the component result (ipython details.diffs /
					// edit details.diff). Do NOT merge into agentRunFileChanges — that map is
					// workspace-level only (turn/uncommitted), updated via workspace_file_changes.
			...
			case "workspace_file_changes": {
				// Workspace-level accumulated view (turn baseline or uncommitted).
				if (event.replace) {
					this.agentRunFileChanges.clear();
				}
				for (const change of event.changes) {
					if (change.added === 0 && change.removed === 0) continue;
					const existing = this.agentRunFileChanges.get(change.path);
					if (existing) {
						existing.added = change.added;
						existing.removed = change.removed;
					} else {
						this.agentRunFileChanges.set(change.path, { ...change });
					}
				}
				this.renderRecap();
				this.ui.requestRender();
			...
			case "turn_end":
				// Workspace-level recap is owned exclusively by `workspace_file_changes`
				// (turn / uncommitted reads). Do not fold per-tool diffs into that map —
				// those stay on the tool result (`details.diffs` / `details.diff`).
				break;
			...
			case "auto_retry_start": {
				// Design A: each failed attempt is a transcript line (full
				// error on the next line); countdown stays in the status area.
				this.showRetryAttemptFailure(
					event.attempt,
					event.maxAttempts,
					event.errorMessage || "Provider request failed",
				);
				this.stopWorkingLoader();
				this.statusContainer.clear();
				this.retryCountdown?.dispose();
			...
			case "auto_retry_end": {
				...
				// Restore the working loader if streaming/subagents still warrant it.
				this.syncWorkingLoader();
				// Final failure: attempts already listed; summarize with full error.
				if (!event.success) {
					const finalError = (event.finalError || "Unknown error").trim() || "Unknown error";
					this.chatContainer.addChild(new Spacer(1));
					this.chatContainer.addChild(
						new Text(
							`${theme.fg("error", `Error: Retry failed after ${event.attempt} attempts`)}\n${theme.fg("error", finalError)}`,
							1,
							0,
						),
					);
	...
	private async returnToAgentsView(request: InteractiveModeRunResult["type"] = "agents_view"): Promise<void> {
		...
		try {
			try {
				if (!this.options.preserveAgentConnectionOnHandoff) {
					await this.agentConnection.dispose();
				}
			} finally {
				await this.options.onShutdown?.();
	 ...
	 * mode and hides the cursor; without this handler, an uncaught throw tears
	 * down the process while leaving the terminal broken (no echo), requiring
	 * `stty sane` / console reset to recover.
	 *
	...
	private registerSignalHandlers(): void {
		...
		for (const signal of signals) {
			const handler = () => {
				// SIGHUP no longer hard-exits: graceful shutdown restores the
				// terminal first. A genuinely dead terminal surfaces as EIO on
				// restore writes, which the stdout/stderr error handler converts
				// into emergencyTerminalExit.
				killTrackedDetachedChildren();
				void this.shutdown();
			};
	...
	/** Design A retry UX: one transcript entry per failed attempt (full message, no truncate). */
	showRetryAttemptFailure(attempt: number, maxAttempts: number, errorMessage: string): void {
		const reason = errorMessage.trim() || "Provider request failed";
		this.chatContainer.addChild(new Spacer(1));
		this.chatContainer.addChild(
			new Text(
				`${theme.fg("warning", `· Retry ${attempt}/${maxAttempts} failed`)}\n${theme.fg("muted", reason)}`,
				1,
				0,
			),
		);
	...
	private getThemeCompletions(prefix: string): AutocompleteItem[] | null {
		const themes = getAvailableThemes();
		if (themes.length === 0) return null;
		const current = this.settingsManager.getTheme();
		const term = prefix.trim().toLowerCase();
		const matches = term ? themes.filter((name) => name.startsWith(term)) : themes;
		if (matches.length === 0) return null;
	...
	private handleThemeCommand(arg: string): void {
		...
		if (!themes.includes(requested)) {
			this.showError(`Unknown theme '${requested}'. Available: ${themes.join(", ")}`);
			return;
		}
	...
	private showThemeSelector(): void {
		...
		const themes = getAvailableThemes();
		const currentTheme = themes.includes(originalTheme) ? originalTheme : (themes[0] ?? "dark");
		this.showSelector((done) => {
			const selector = new ThemeSelectorComponent(
				...
				() => {
					const result = setTheme(originalTheme, true);
					if (result.success) {
						this.ui.invalidate();
					}
					done();
	...
	private createAuthFlows(): ProviderAuthFlows {
		return new ProviderAuthFlows({
			...
			excludedLoginProviderIds: this.options.excludedLoginProviderIds,
			onAuthChanged: async () => {
			...
			if (result.changed && result.serverChange) {
				...
				const successMessage =
					verb === "removed"
						...
						: usesOAuth
							? `${verb === "replaced" ? "Replaced" : "Added"} MCP server "${name}" (${transport}). ${hasMcpProviderRefresh ? "Run" : "Restart Devo, then run"} /mcp login ${name} to connect.`
	...
	private async handleSystemPromptCommand(): Promise<void> {
		...
		const component = new SystemPromptComponent(prompt, this.toolOutputExpanded);
		this.chatContainer.addChild(component);
		this.ui.requestRender();
	}
	...
	private async handleHeartbeatCommand(text: string): Promise<void> {
		try {
			const args = text.replace(/^\/heartbeat\b/i, "").trim();
			const result = await this.agentConnection.runHeartbeatCommand(args);
			const heartbeat = result.job;
			switch (result.action) {
				...
				case "set": {
					if (!heartbeat) {
						this.showError("Heartbeat was not set");
						return;
					}
