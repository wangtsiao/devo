...
export interface ProviderAuthFlowsHost {
	 ...
	 * Used by rebranded hosts (e.g. Devo) that should not surface Prime Inference chrome.
	 */
...
export class ProviderAuthFlows {
	...
	getLoginProviderOptions(authType?: "oauth" | "api_key"): AuthSelectorProvider[] {
		...
		const excluded = new Set(this.host.excludedLoginProviderIds ?? []);
	...
	async runPrimeAgentTracesLogin(): Promise<AuthenticationResult> {
		...
		try {
			...
			if (result.source === "manual") {
				browserAbort.abort();
				dialog.showProgress("Checking Devo trace access...");
				...
				if (!access.ok) {
					const status = access.status === undefined ? "" : `HTTP ${access.status}: `;
					throw new Error(`Prime API key does not have Devo trace access (${status}${access.message})`);
				}
			}
