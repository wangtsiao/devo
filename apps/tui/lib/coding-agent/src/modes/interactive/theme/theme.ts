...
export function getAvailableThemes(): string[] {
	...
	// Devo: hide the Prime brand theme from the picker (file remains loadable for
	// existing settings that already selected it). Only dark/light are offered.
	themes.delete("prime");
	const allowed = ["dark", "light"].filter((name) => themes.has(name));
	return allowed.length > 0 ? allowed : ["dark", "light"];
}
...
export function getAvailableThemesWithPaths(): ThemeInfo[] {
	const themesDir = getThemesDir();
	// Devo: only dark/light in the picker (Prime brand theme and customs stay loadable).
	return ["dark", "light"].map((name) => ({ name, path: path.join(themesDir, `${name}.json`) }));
}
...
function getDefaultTheme(): string {
	// Devo: dark/light only in the picker; default dark for dark terminals.
	return detectTerminalBackground() === "light" ? "light" : "dark";
}
