...
export type AgentConnectionSessionEvent =
	...
	/** Tool discovered while model streams arguments — create UI, do not mark running. */
	| { type: "tool_pending"; toolCallId: string; toolName: string; args: any }
	/** Progressive tool-argument update (partial JSON → object). */
	| { type: "tool_args_update"; toolCallId: string; toolName?: string; args: any }
	/** Arguments finished assembling; edit tools may compute diffs. */
	| { type: "tool_args_complete"; toolCallId?: string }
	/** Turn-scoped workspace file changes (from git/fs baseline) after a tool. */
	| {
...
export interface AgentConnection {
	...
	runHeartbeatCommand(args: string): Promise<import("../../core/cron-jobs.js").HeartbeatCommandResult>;
	setHeartbeat(
		schedule: string,
		instruction: string,
